...
[assembly: CommandClass(typeof(Justified.SpecificationReflow.AutoCAD.PluginHost.DevNoteCommands))]

namespace Justified.SpecificationReflow.AutoCAD.PluginHost;

// 开发核对命令：每次选 Word、草案目录、图幅和单位比例，草案的 pending 只在内存补齐，磁盘不改。
// 正式人员操作不用这个命令，正式两步是 DN_NOTE_SET + DN_NOTE。
public class DevNoteCommands
{
    [CommandMethod("DN_NOTE_DEV", CommandFlags.Modal)]
    public void Note()
    {
        var document = CadApplication.DocumentManager.MdiActiveDocument;
        if (document == null) return;
        var editor = document.Editor;
        var before = NoteCommands.Count(document.Database);
        try
        {
            ...
            editor.WriteMessage("\nDN_NOTE_DEV_DOCX " + file.StringResult);
            editor.WriteMessage("\nDN_NOTE_DEV_ANCHOR_UCS " + Format(point.Value.X) + "," + Format(point.Value.Y) + "," + Format(point.Value.Z));
            editor.WriteMessage("\nDN_NOTE_DEV_ANCHOR_WCS " + Format(wcs.X) + "," + Format(wcs.Y) + "," + Format(wcs.Z));
            ...
            foreach (var diagnostic in session.Diagnostics.Where(item => item.Severity == Severity.Info))
                editor.WriteMessage("\nDN_NOTE_DEV_DRAFT " + diagnostic.Message);
            if (!session.Success || session.Standard == null || session.Template == null)
            {
                foreach (var diagnostic in session.Diagnostics.Where(item => item.Severity == Severity.Error))
                    editor.WriteMessage("\nDN_NOTE_DEV_FAILED " + diagnostic.Code + " " + diagnostic.Message);
                editor.WriteMessage("\nDN_NOTE_DEV_ENTITIES before=" + before + " after=" + NoteCommands.Count(document.Database) + "\n");
                return;
            }

            editor.WriteMessage("\nDN_NOTE_DEV_STANDARD " + session.Standard.StandardId + " " + session.Standard.Version);
            editor.WriteMessage("\nDN_NOTE_DEV_TEMPLATE " + session.Template.TemplateId + " " + session.Template.Version);
            editor.WriteMessage("\nDN_NOTE_DEV_SCALE " + Format(scale.Value));
            ...
            foreach (var diagnostic in generated.Diagnostics.Where(item => item.Severity == Severity.Warning))
                editor.WriteMessage("\nDN_NOTE_DEV_WARN " + diagnostic.Code + " " + diagnostic.Message);
            ...
            if (generated.Texts.Count > 0)
            {
                ...
                editor.WriteMessage("\nDN_NOTE_DEV_FIRST x=" + Format(first.Position.X) + " y=" + Format(first.Position.Y)
            ...
            if (!report.Success)
            {
                foreach (var diagnostic in report.Diagnostics)
                    editor.WriteMessage("\nDN_NOTE_DEV_FAILED " + diagnostic.Code + " " + diagnostic.Message);
                editor.WriteMessage("\nDN_NOTE_DEV_ENTITIES before=" + before + " after=" + after + "\n");
                return;
            }

            editor.WriteMessage("\nDN_NOTE_DEV_ENTITIES before=" + before + " after=" + after);
            editor.WriteMessage("\nDN_NOTE_DEV_OK objects=" + report.ObjectCount);
            editor.WriteMessage("\nDN_NOTE_DEV_UNDO 一次 U 撤销本次提交。已有同名样式不会被修改，旧文字也不会被删除。\n");
        }
        catch (System.Exception error)
        {
            editor.WriteMessage("\nDN_NOTE_DEV_FAILED " + error.GetType().Name + " " + error.Message);
            editor.WriteMessage("\nDN_NOTE_DEV_ENTITIES before=" + before + " after=" + NoteCommands.Count(document.Database) + "\n");
        }
    }
