...
- NFR-006 可维护性：发布包包含版本、支持环境清单、标准模板包及部署/回滚说明；不要求开发机路径或联网安装依赖。
- NFR-007 可取消：耗时阶段提供进度和取消；实际取消时延 CAL-08 标定。事务提交期间不可安全取消时显示正在完成，失败仍须回滚。

## 12 验收标准与追踪矩阵

发布门槛：所有 MUST 功能通过；无未解决的 error；标定数据齐备；性能目标按最终样本实测；结构专业业务人员完成图面核验。不能用模拟测量单测替代实际 AutoCAD 字体与打印核验。
...
| AC-06 | Φ ± ℃ ≤ ≥ ² ³、上下标及 20～50 条真实句子 | 屏幕与打印字形正确；测量与实体边界误差在标定内 | TEXT-001 |
