...
- AP-004 固定标准：院标内置为版本化配置资产，不要求用户每次设置，不从当前 DWG 现有文字推断院标。A1/A2/A3 各有固定 Layout 模板，不提供用户模板编辑器或矩形识别流程。
...
编辑并保存 DOCX → 在 CAD 启动插入命令 → 选择文件与 A1/A2/A3 → 解析、校验、排版 → 有错误则修复；有警告则查看并明确继续 → 点击 Anchor → 一次事务生成全部页 → 显示完成摘要。

无警告的常规路径不增加额外确认。选择文件、处理警告或点击 Anchor 时取消，应终止本次操作；不得残留半份文字。再次生成不查找旧对象、不删除任何已有对象。
