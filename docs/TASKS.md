...
视觉稿：[CAD 导入说明窗口](assets/cad-note-picker-concept-v1.png)。这是设计提案，不是已实现截图。用户提供的三段式几何 logo 仅用于图形参考，品牌中文名仍待确定；不要把草图中的名称写死在程序或安装包里。

- 目标是极简、现代、少文字但易懂：图标配短标签，不用仅有图标的隐语；浅色卡片、黑灰文字、单一蓝色主操作色。桌面端未来沿用颜色、字体、间距、图标与状态文案，CAD 端保持紧凑。
- 操作顺序：选 DOCX → 选 A1/A2/A3（卡片内画出 3/3/2 栏）→ 核对模板版本和单位比例 → 点击“下一步·在图纸中点位置”。标准包显示已选版本并可切换；上次选择可预填但每次可改。
