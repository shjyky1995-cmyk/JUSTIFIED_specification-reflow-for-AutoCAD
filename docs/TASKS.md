...
| T16/D1 | 固定模板填写 → 生成并检查 DOCX 的独立桌面工作台 | T15/D0 | desktop/app、desktop/worker、集成检查、设计规范 | 桌面 PRD D1 | 实际桌面窗口完成填写、生成、打开、检查；Word/WPS 可编辑；生成文件可被现有解析器读回；不覆盖已有文件 | 进行中（工作进程与前端构建已通过；真实窗口按钮与视觉待验证，当前为 checkpoint） |
...
3. **后续桌面应用**：现有 PRD 将独立桌面工作台列为 CAD V1 以外。首批流程现已确定为固定模板生成 DOCX、Word/WPS 修改、用户在 CAD 重新选择导入。界面技术按用户委托选择 Electron，详见桌面 PRD 与 ADR-013；先做可运行窗口，再实测。不得预建空服务或把桌面端计入 CAD V1 完成度。
