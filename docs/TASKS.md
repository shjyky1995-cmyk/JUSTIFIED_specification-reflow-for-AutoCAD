...
当前任务：阶段3 T12/T14 进行中，工作分支 task/T14-note-picker（依赖未验收的 T12，未合 main）。外机已验证同图 A1/A2/A3 落图；界面改版与耗时实测列为发布前工作，详见 NEXT_AGENT。
...
| T14/M5 | 每次选择图幅/版本/DOCX → CAD 小窗口与混合落图 | T10；宿主验收依赖 T12 的已发布候选资源 | PluginHost、操作文档、宿主验证记录 | PRD 3.3、UX-001 | 同一空白 DWG 连续选择 A1/A2/A3 与不同 DOCX，分别点位生成；再次打开可改选；取消不新增对象 | 进行中（外机三图幅同图生成及窗口取消已验证；窗口外观被用户指出需改版；不同 DOCX、点位取消待验） |
| T13/M5 | 候选包与已批准标准 → 普通用户安装包 | T12 正式验收、T14 宿主验收、标准资产批准 | 安装/升级/卸载入口、打包脚本、用户指南 | NFR-006、PRD 3.3 | 全新 Windows 用户从单一发布包完成图形化安装；不使用 GitHub/PowerShell/手工复制；AutoCAD 2021 重启后可加载，异常与回退路径实测 | 待开始（用户 2026-09-24 明确提出推广安装体验） |
