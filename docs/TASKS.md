...
| T13/M5 | 候选包与已批准标准 → 普通用户安装包 | T12 正式验收、标准资产批准 | 安装/升级/卸载入口、打包脚本、用户指南 | NFR-006、PRD 3.3 | 全新 Windows 用户从单一发布包完成图形化安装；不使用 GitHub/PowerShell/手工复制；AutoCAD 2021 重启后可加载，异常与回退路径实测 | 待开始（用户 2026-09-24 明确提出推广安装体验） |
