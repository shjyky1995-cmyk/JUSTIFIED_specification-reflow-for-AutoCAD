...
| T14/M5 | 每次选择图幅/版本/DOCX → CAD 小窗口与混合落图 | T10；宿主验收依赖 T12 的已发布候选资源 | PluginHost、操作文档、宿主验证记录 | PRD 3.3、UX-001 | 同一空白 DWG 连续选择 A1/A2/A3 与不同 DOCX，分别点位生成；再次打开可改选；取消不新增对象 | 进行中（外机三图幅同图生成及旧窗口取消已验证；0925 修复版离线预览已核，CAD 宿主待部署复测；不同 DOCX、点位取消待验） |
