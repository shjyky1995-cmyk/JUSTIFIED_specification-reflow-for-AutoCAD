...
当前任务：阶段3 T12 进行中，分支 task/T12-acceptance；T01～T11 已合入 main，标签 mvp-1.0。T12 需完成三图幅业务、外机、打印、断网、取消时延及专业签收，见 NEXT_AGENT。
...
| T12/M5 | 三图幅/业务样本 → 签收与成效记录 | T11、全部 CAL | 验收记录、脱敏样本、操作/演示文档 | AC-01～14 | 本机+外机、屏幕+打印、业务复核 | 进行中（CAL-06 WPS A1 已核对；三图幅正式入口待验） |
| T14/M5 | 每次选择图幅/版本/DOCX → CAD 小窗口与混合落图 | T10；宿主验收依赖 T12 的已发布候选资源 | PluginHost、操作文档、宿主验证记录 | PRD 3.3、UX-001 | 同一空白 DWG 连续选择 A1/A2/A3 与不同 DOCX，分别点位生成；再次打开可改选；取消不新增对象 | 进行中（代码构建及核心检查通过，宿主 UI/混合图面待验） |
