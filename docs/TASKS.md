...
| T15/D0 | 桌面 DOCX 编制工作台 → 技术框架与开发路线定稿 | CAD v0.1.0 已签收；打印修复待复核 | 桌面 PRD 草案、架构比较、分阶段路线、用户决策记录 | 桌面 PRD D0 | 固定模板生成 DOCX、Word/WPS 修改、CAD 直接导入；确定 UI 技术后才启动 D1 | 进行中（单 DOCX 流程已确定；WPF/.NET 10 与 Electron 已比较，技术选型待用户定稿） |
