...
| T15/D0 | 桌面导入工作台 → 技术框架与开发路线定稿 | CAD v0.1.0 已签收 | 桌面 PRD 草案、架构比较、分阶段路线、用户决策记录 | 桌面 PRD D0 | 明确首批范围、UI 技术、CAD 交接和页数边界；用户确认后才启动 D1 | 进行中（仅方案；WPF/.NET 10 与 Electron 已比较，推荐 WPF；等待用户讨论定稿） |
