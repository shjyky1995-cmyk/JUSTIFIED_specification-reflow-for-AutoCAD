# 任务清单
默认一次对话推进一个可独立检查的小任务（Txx-yy）；阶段任务表不等于一轮必须做完整阶段。固定交付/验收按 [WORKFLOW](WORKFLOW.md)，唯一下一步读 [NEXT_AGENT](NEXT_AGENT.md)。任务范围不授权扩大 PRD。
领取时记录负责人、输入/输出与细分步骤到 docs/devlog/<编号>.md。
状态：待开始、进行中、阻塞、待验收、完成。无真实证据不得完成。
当前任务：T06 标准校验与测量原型（待验收，分支 task/T06-standards-measure，未合入 main）；用户授权T07先行开发，下一接续见 [交接](NEXT_AGENT.md)。T05 编号与空白审计已完成并合入 main（c8e75fd）。
...
2026-09-22 流程完善：当前唯一下一小任务为T07-01基础自动换行，纯核心算法，无需用户操作CAD；收到继续下一步直接领取执行。其余接续状态由NEXT_AGENT维护，避免多处重复保存具体下一步。
