...
2026-09-26 接续：CAD v0.1.0 已发布，用户选择桌面首批场景为“离线导入工作台”，要求先定技术框架与开发路线再编码。比较和推荐方案写在 [桌面导入工作台 PRD 草案](DESKTOP_WORKBENCH_PRD_DRAFT.md)；本段历史提案不代表 Electron 已被批准，草案也不改变当前模块边界。
...
采用 Electron 会新增 JavaScript/Node 构建与打包、额外运行体积及安全维护责任，属于技术栈变化。进入桌面阶段前，先明确桌面端首批实际业务场景，再做一个离线运行的小原型，比较交互效果、包体、启动/内存、安装/卸载和与 C# 核心通信；记录取舍并请用户确认。此提案不修改当前模块依赖或 V1 验收条件。
## 模块与允许引用
