...
2026-09-25 更新：用户已确认模板版本由管理员维护；新版窗口只显示 Word、图幅、单位比例。修复窗口比例、logo 与 `N/mm²` 的候选须由 Agent 安装后重启 CAD，再按以下步骤复测。
...
2. 输入 `DN_NOTE`。首次如提示缺模板，点“选择模板目录”，指向 `artifacts/t14-share/20260924-fix-43fa507/local-test-package`。Word 文件选同目录上一级的 `sample.docx`，图幅选 A2；模板版本不在日常窗口出现。
